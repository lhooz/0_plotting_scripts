...
def cf_plotter(pa, data_array, legends, time_to_plot, coeffs_show_range,
    ...
    for axrow in ax:
        ...
        if coeffs_show_range != 'all':
            axrow[0].set_ylim(coeffs_show_range[0])
            axrow[1].set_ylim(coeffs_show_range[1])
        ...
        if plot_mode != 'wake':
            if axrow[0] == ax[0][0]:
                axrow[0].legend(loc='upper center',
                                bbox_to_anchor=(1.05, 1.2),
                                ncol=4,
                                fontsize='small',
                                frameon=False)

        markx_loc = axrow[0].get_xlim()[0] + 0.5 * (axrow[0].get_xlim()[1] -
                                                     axrow[0].get_xlim()[0])
        markymid_loc = axrow[0].get_ylim()[1] + 0.15 * (axrow[0].get_ylim()[1] -
                                                       axrow[0].get_ylim()[0])

        axrow[0].annotate(s=legendre[rowno],
