...
def cf_plotter(data_array, legends, time_to_plot, show_range, image_out_path,
               cycle_time, AR, pt, at_folder, plot_mode):
    """
    ...
    title = 'transient_pt_AR' + '{:.0f}'.format(AR * 10) +'_at' + '{:.0f}'.format(at_folder)
    ...
    if plot_mode == 'against_t':
        for i in range(len(legends)):
            ...
            if pt[i] < 0.5:
                mclt = cl_spl.integral(time_to_plot[0] + 0.5 * pt[i], time_to_plot[1] - 0.5 - 0.5 * pt[i]) / (0.5 - pt[i])
                mclr = cl_spl.integral(time_to_plot[0] + 0.5 - 0.5 * pt[i], time_to_plot[0] + 0.5 + 0.5 * pt[i]) / pt[i]
            else:
                mclt = 0
                mclr = mcl
            ...
            if pt[i] < 0.5:
                mcdt = cd_spl.integral(time_to_plot[0] + 0.5 * pt[i], time_to_plot[1] - 0.5 - 0.5 * pt[i]) / (0.5 - pt[i])
                mcdr = cd_spl.integral(time_to_plot[0] + 0.5 - 0.5 * pt[i], time_to_plot[0] + 0.5 + 0.5 * pt[i]) / pt[i]
            else:
                mcdt = 0
                mcdr = mcd
