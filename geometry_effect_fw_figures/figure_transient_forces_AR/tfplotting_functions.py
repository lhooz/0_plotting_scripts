...
def cf_plotter(data_array, marks, time_to_plot, show_range, image_out_path,
    ...
    if plot_mode == 'against_t':
        for rei in range(len(markre)):
            ax = axs[:, rei]
            datano = rei * len(markr1h) * len(legends)
            for i in range(len(markr1h) * len(legends)):
                ax_mk = int(i / 4)
                legendi = np.mod(i, 4)
    ...
    title = 'force coefficients plot cl'
    ...
    title2 = 'force coefficients plot cd'
