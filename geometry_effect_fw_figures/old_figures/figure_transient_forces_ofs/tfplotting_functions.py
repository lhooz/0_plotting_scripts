...
def cf_plotter(data_array, marks, time_to_plot, show_range, image_out_path,
    ...
    if plot_mode == 'against_t':
        for rei in range(len(markre)):
            ax = axs[:, rei]
            datano = rei * len(markar) * len(legends)
            for i in range(len(markar) * len(legends)):
                ax_mk = int(i / 3)
                legendi = np.mod(i, 3)
    ...
    title2 = 'force coefficients plot cm'
