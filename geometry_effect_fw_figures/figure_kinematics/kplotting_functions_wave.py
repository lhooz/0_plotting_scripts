...
def k_plotter(karray_all, time_scale, time_to_plot, show_range, image_out_path,
    ...
    for kine_array, lgd1, lgd2 in zip(karray_all, legendax1, legendax2):
        ...
        ax[1].plot(kine_array[:, 0] / time_scale, kine_array[:, 2])
    ...
    for axi, labeli, rangei in zip(ax, labels, show_range):
    ...
    ax[1].set_yticks([45, 90])

    ax[1].axhline(y=45, color='k', linestyle='-.', linewidth=0.5)
