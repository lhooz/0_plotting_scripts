...
def field_plot(windows, ufield_gridx, ufield_gridy, sImgfield_data,
    ...
    window = windows
    no_r = int(len(wgeo_data) / 2)
    no_c = 2
    imnorm = colors.Normalize(vmin=-5e2, vmax=5e2)
    levels = np.linspace(9.0e2, 5.0e4, 50)
    zoom_order = 5
    ...
    fig, axs = plt.subplots(nrows=no_r, ncols=no_c, gridspec_kw=gs_kw)
    ax_stack_col = np.concatenate((axs[:, 0], axs[:, 1]))
    for grid_x, grid_y, sImgdatai, sCtrdatai, vdatai, wdatai, axr1i, marksci in zip(
            ...
            vfield_data, wgeo_data, ax_stack_col, marksc):
        ...
        markx_loc = axr1i.get_xlim()[0] - 0.1 * (axr1i.get_xlim()[1] -
                                                 axr1i.get_xlim()[0])
